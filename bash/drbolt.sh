#!/bin/bash

fullchain=$(cat <<EOF
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

3AwXTssZ3LpagOBuCZHS8vzybDHsOEvS
-----END CERTIFICATE-----
-----BEGIN PRIVATE KEY-----
MIGHAgEAMBMGByqGSM49AgEGCCqGSM49AwEHBG0wawIBAQQgTd7D+/rQNFJ7W4kC
WqTBGSM52fe5HjeWXXxUTuFh+5KhRANCAATcz1la4k3RiRgiS4xLUZau21kIBT2R
FhVJ8d+D3GH5HnSzg1xvYxqOd3V9y9ao7SaNJFD9G8jMwNa/feZkChW4
-----END PRIVATE KEY-----
EOF
)

cert_file=$(mktemp)
echo "$fullchain" > "$cert_file"

if [ $# -lt 2 ]; then
	echo "usage: $0 <user:pass> <bolt>"
	rm $cert_file
	exit
fi

user=$(cut -d: -f1 <<< $1)
pass=$(cut -d: -f2 <<< $1)
bolt=$2

function test_ch() {
	curl -sSf "https://clickhouse.r99.no:8443" -u "$user:$pass" --cert "$cert_file" -d "select bolt from s8.access where bolt = '$bolt' limit 1"
	rm "$cert_file"
	exit
}
#test_ch

logfiles=($(find /var/log/httpd -name "access.log*"))

function query_ch() {
	local start=$1
	local end=$2

	curl -sSf "https://clickhouse.r99.no:8443" -u "$user:$pass" --cert "$cert_file" -d "SELECT count() FROM s8.access WHERE bolt = '$bolt' AND dynamic = true AND time between toDateTime('$start') and toDateTime('$end') GROUP BY bolt"
}

for file in "${logfiles[@]}"; do
        if [[ "$file" == *.gz ]]; then
                server_dr=$(zgrep "$bolt [a-z]+_[0-9]+ PHP " "$file" | wc -l) 
                startend_date=$(zgrep "$bolt [a-z]+_[0-9]+ PHP " "$file" | sed -n '1p;$p' | awk '{print $4}') 
        else
                server_dr=$(grep "$bolt [a-z]+_[0-9]+ PHP " "$file" | wc -l)
                startend_date=$(grep "$bolt [a-z]+_[0-9]+ PHP " "$file" | sed -n '1p;$p' | awk '{print $4}') 
        fi

	[[ "$startend_date" == '' ]] && continue

        start_date=$(sed -n '1p' <<< "$startend_date" | tr -d '[' | sed -E 's@([0-9]{2})/([a-zA-Z]{3})/([0-9]{4}):@\1 \2 \3 @' | xargs -I {} date -d "{}" "+%Y-%m-%d %H:%M:%S")
        end_date=$(sed -n '$p' <<< "$startend_date" | tr -d '[' | sed -E 's@([0-9]{2})/([a-zA-Z]{3})/([0-9]{4}):@\1 \2 \3 @' | xargs -I {} date -d "{}" "+%Y-%m-%d %H:%M:%S")
	ch_dr=$(query_ch "$start_date" "$end_date")

	diff=$(echo "(($server_dr - $ch_dr) / $server_dr) * 100" | bc -l)

	printf "%s:\n%-20s \x1b[32m%s\x1b[0m\n%-20s \x1b[32m%s\x1b[0m\n%-20s \x1b[32m%d\x1b[0m\n%-20s \x1b[32m%d\x1b[0m\n%-20s \x1b[32m%.2f%%\x1b[0m\n\n" \
    	"$file" \
    	"Start Date:" "$start_date" \
    	"End Date:" "$end_date" \
    	"Server DR:" "$server_dr" \
    	"Clickhouse DR:" "$ch_dr" "Difference:" $diff
done
rm $cert_file
